...
_ACTION_CLIP = np.array([-0.02, 0.02, -0.02, 0.02, -0.01, 0.01], dtype=np.float32)
...
class PushingEnv(gym.Env):
  ...
  def reset(  # type: ignore[override]
    ...
    *,
    ...
    if config is None:
      raise ValueError("PushingEnv.reset() requires a SceneConfig argument.")
    self.config = config
  ...
  def step(
    ...
    # Step physics
    n_substeps = max(1, int(round(self.config.control_dt / self.model.opt.timestep)))
  ...
  def _get_object_pos(self) -> np.ndarray:
    ...
    body_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "target_object")
